...
export default async function ZipResultsPage({ params, searchParams }: Props) {
  ...
  const limit = Math.min(Math.max(Number(searchParamsData?.limit ?? 100), 1), 500);

  const supabase = getSupabaseServer();
  const { data, error } = await supabase.rpc("providers_near_zip", {
    p_zip: zip,
        ...
        <div className="mx-auto max-w-6xl px-4 py-12">
          <div className="bg-white rounded-xl p-8 text-center">
            ...
            <p className="text-ink/60 mb-4">
              This might mean the ZIP code wasn't found in our database, or there was a connection issue.
            </p>
